...
model = Sequential()
model.add(Conv1D(filters=16, kernel_size=1, activation='relu', input_shape=(X_train_cnn.shape[1], 1)))
...
history = model.fit(X_train_cnn, y_train, epochs=50, batch_size=8, verbose=1)
...
sample_cnn = sample_sc.reshape((sample_sc.shape[0], sample_sc.shape[1], 1))

sample_pred = model.predict(sample_cnn)
